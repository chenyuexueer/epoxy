...
internal class ModelViewWriter(
    ...
    fun writeModels(models: List<ModelViewInfo>) {
        for (modelInfo in models) {
            try {
                modelWriter.generateClassForModel(modelInfo, object : GeneratedModelWriter.BuilderHooks() {
                    internal override fun addToBindMethod(methodBuilder: MethodSpec.Builder, boundObjectParam: ParameterSpec): Boolean {
                        ...
                        addBindStyleCodeIfNeeded(modelInfo, methodBuilder, boundObjectParam, hasPreviousModel = false)

                        return true
                    }

                    internal override fun addToBindWithDiffMethod(methodBuilder: MethodSpec.Builder, boundObjectParam: ParameterSpec,
                        ...
                        addBindStyleCodeIfNeeded(modelInfo, methodBuilder, boundObjectParam, hasPreviousModel = true)

                        return true
                    }

                    override fun addToHandlePostBindMethod(postBindBuilder: MethodSpec.Builder,
    ...
    private fun addBindStyleCodeIfNeeded(
        ...
        methodBuilder.apply {
            ...
            addStatement("\$T styleApplier = new \$T(\$L)",
                    styleInfo.styleApplierClass, styleInfo.styleApplierClass, boundObjectParam.name)

            addStatement("styleApplier.apply(\$L)", PARIS_STYLE_ATTR_NAME)

//            if (configManager.shouldValidateModelUsage()) {
//                addStatement("\$T.assertSameAttributes(styleApplier, \$L, \$L)",
//                        ClassNames.PARIS_STYLE_UTILS, PARIS_STYLE_ATTR_NAME, PARIS_DEFAULT_STYLE_CONSTANT_NAME)
//            }
            ...
            addStatement("\$L.setTag(\$T.id.epoxy_saved_view_style, \$L)",
                    boundObjectParam.name, ClassNames.EPOXY_R, PARIS_STYLE_ATTR_NAME)
    ...
    fun modelImplementsBindWithDiff(clazz: TypeElement, bindWithDiffMethod: MethodSpec): Boolean {
        val methodOnClass = Utils.getMethodOnClass(clazz, bindWithDiffMethod, types, elements) ?: return false
    ...
    private fun addResetMethodsToBuilder(builder: MethodSpec.Builder, modelViewInfo: ModelViewInfo,
    ...
    private fun addAfterPropsAddedMethodsToBuilder(methodBuilder: MethodSpec.Builder, modelInfo: ModelViewInfo,
                                                   boundObjectParam: ParameterSpec) {
