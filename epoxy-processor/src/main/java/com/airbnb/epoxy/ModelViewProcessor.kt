...
internal class ModelViewProcessor(
        ...
        private val layoutResourceProcessor: LayoutResourceProcessor
) {
    ...
    private fun processViewAnnotations(roundEnv: RoundEnvironment) {
        for (viewElement in roundEnv.getElementsAnnotatedWith(ModelView::class.java)) {
            try {
                ...
                val modelViewInfo = ModelViewInfo(viewElement as TypeElement, types, elements, errorLogger,
                        configManager, layoutResourceProcessor)

                modelClassMap.put(viewElement, modelViewInfo)
            } catch (e: Exception) {
